# Summer Geometry Initiative: TinyAD + Polyscope starter
...
You do **not** need to understand the Geometry Processing Lab web application
to use this folder. You can copy `student-starter/` into a new repository and
build it on its own.
...
./build/sgi-tinyad-starter
```

On Windows with a multi-configuration generator, the executable may instead be
under `build/Release/`.

The first build is the slow one because it downloads and compiles the pinned
dependencies. Later builds of `main.cpp` should be much faster.

If you are working inside the full Geometry Processing Lab repository, you can
reuse its native build:

```sh
cmake --preset native
cmake --build --preset native --target sgi-tinyad-starter
./build/native/student-starter/sgi-tinyad-starter
```
...
```sh
cp -R student-starter ../my-geometry-project
...
Run those commands from the Geometry Processing Lab repository root. Rename
...
```sh
./build/sgi-tinyad-starter --headless
```
...
Use the full Geometry Processing Lab when you need reusable mesh loading,
